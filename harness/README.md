...
- `journey-plugins`：插件纵向产品链。旧 MCP journey 已退役，新组件测试随实现建立。
...
三平台原生构建共用一次 `cargo build --locked`，构建步骤上限为 15 分钟，整个原生 CI job 上限为 60 分钟，
为冷构建和后续平台验证留出余量；不重试，也不叠加 Linux 专属的 180 秒截止。
构建检查验证能否编译，不把共享 runner 的冷构建速度当作产品性能契约。
PR run `34684619702` 与 main run `34685095387` 的文件内容、Ubuntu 镜像和 Rust/Cargo 版本一致，
前者约 131 秒完成，后者编译进度较慢并在第 180 秒被旧内层期限终止（退出码 124），尚未进入测试。
这类失败应核对各阶段耗时及退出原因；取消重复期限不代表编译或后续测试已经通过，仍需原平台验证。
