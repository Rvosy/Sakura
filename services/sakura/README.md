...
官网源码也在本目录的 [site/](site/README.md) 维护，使用 Astro / Starlight，独立构建和发布。
`sakura.cialloo.cn` 保留官网，`adm.sakura.cialloo.cn` 为私人控制台，`api.sakura.cialloo.cn` 为公开接口。
目录与线上进程对应关系、配置核对结果见 [部署说明](deploy/README.md)。Python 部署包不包含官网产物。
