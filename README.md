...
`0.9.9-dev` 已把 Tauri 切为唯一生产桌面入口：Tauri 负责桌宠与次级窗口、托盘、截图、音频播放和进程监管，长期运行的 Python Brain Host 继续负责角色、聊天、记忆、工具、TTS 合成、插件、MCP 与主动互动。现有 `data/`、`characters/` 和插件配置格式保持兼容，正常启动路径不需要 PySide6。

## 功能特性

**角色与外观**
- 角色包驱动 — `.char` 一键导入，角色卡、立绘、GPT-SoVITS 语音权重一体打包；UI 主题色跟随角色
- 角色工作室 — 工作室、设置、历史和诊断均由同一 Tauri 应用创建，可新建、编辑并导出 `.char` 角色包
- 首次配置入口 — 缺少角色或模型时可从桌宠菜单打开设置与角色工作室；已有配置直接沿用
...
- 权限确认 — 高风险工具（打开网页、文件操作等）执行前弹出确认面板；第一阶段继续沿用现有 Python 工具策略
...
> 第一阶段仍使用兼容的 Python 工具权限与插件模型。Capability Broker、Permission Manager、插件沙箱和 Credential Broker 尚未实现，不能把当前边界理解为最终安全沙箱。
