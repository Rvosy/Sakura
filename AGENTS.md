...
- 测试应证明用户可观察行为、数据或进程安全、公共契约，或者覆盖已经发生的缺陷。
- 不为实现细节、文件存在、CSS 数值、函数排列或完整文案单独添加测试。外部协议字段、稳定错误码、安全清洗和明确规定的输出格式可以作为契约断言。
- 同一风险只保留一个主要证据层，优先测试最接近真实调用链、最能复现故障的位置。
- 参数化样本只在进入不同逻辑分支或代表不同错误类别时保留；同一分支的输入变体不逐项罗列。
- 修复缺陷时，增加能让原缺陷真实失败的最小回归测试。已有更强证据覆盖同一风险后，删除较弱或重复的旧测试。
