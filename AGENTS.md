...
- 可读写范围内只修改完成任务必需的文件。
- `data/` 是应用的正常可写运行时目录。任务、测试或产品链路需要时可以写入，不得自行附加
  “整个 `data/` 只读”或“全目录 path/mtime/hash 必须零变化”的通用门禁。写入仍须属于任务范围并
  遵循现有数据所有权、原子写、schema、凭据脱敏和共享锁契约；不得清理、截断、恢复或删除无关
  用户数据，破坏性故障注入使用隔离临时目录。
