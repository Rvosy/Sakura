...
def _manager(tmp_path: Path, *, plugin_ids=(PLUGIN,)) -> PluginRuntimeManager:
...
class Plugin:
    def setup(self, context):
        context.provide("{plugin_id}.service", Service(), exports=("child", "crash", "ping"))
''', encoding="utf-8",
        )
...
def test_manager_close_waits_for_cleanup_already_started_elsewhere(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch, first_operation: str,
) -> None:
    ...
    try:
        ...
        cleanup = process._close_owned_process
        wait = process.wait_for_cleanup

        def gated_cleanup(*args):
            entered.set()
        ...
        def invoke(operation, *, final=False):
            try:
                operation()
        ...
        monkeypatch.setattr(process, "_close_owned_process", gated_cleanup)
        monkeypatch.setattr(process, "wait_for_cleanup", observed_wait)
        first = manager.close if first_operation == "close" else lambda: manager.set_enabled(PLUGIN, False)
        worker = threading.Thread(target=invoke, args=(first,))
        workers.append(worker)
        worker.start()
        assert entered.wait(2)
        assert PLUGIN in manager._draining_processes
        worker = threading.Thread(target=invoke, args=(manager.close,), kwargs={"final": True})
        workers.append(worker)
        worker.start()
        assert observed.wait(2)
        assert waiting.is_set(), "Core close must join an in-flight plugin cleanup"
        ...
        assert manager._draining_processes == {}
...
def test_manager_close_finishes_other_plugins_before_reporting_cleanup_failure(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch,
) -> None:
    manager = _manager(tmp_path, plugin_ids=(PLUGIN, "fixture.other"))
    processes = []
    child = None
    failure = OSError("fixture cleanup failure after process exit")
    try:
        manager.start()
        failed_id = manager._activation_order[-1]
        healthy_id = manager._activation_order[0]
        failed = manager._records[failed_id].process
        healthy = manager._records[healthy_id].process
        processes = [failed, healthy]
        child = psutil.Process(manager.call_service(f"{healthy_id}.service", "child"))
        cleanup = failed._close_owned_process

        def failed_cleanup(*args):
            cleanup(*args)
            raise failure

        monkeypatch.setattr(failed, "_close_owned_process", failed_cleanup)
...
@pytest.mark.parametrize("source", ["process_exit", "manager_close"])
def test_repeated_stop_waits_for_removed_process_cleanup(
    ...
    try:
        ...
        stopping_thread = threading.Thread(target=stop_again)
