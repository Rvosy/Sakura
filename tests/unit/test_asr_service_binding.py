...
def _invalidate_source(application, boundary, character, source):
    if source != "character_session":
        application.reload_plugin(source)
        return
    identities = {key: application.service_identity(key) for key in (SERVICE, "sakura.asr")}
    for character_id in ("another-character", "fixture"):
        with boundary.suspend_for_character_change():
            character["characterId"] = character_id
    assert {key: application.service_identity(key) for key in identities} == identities
...
@pytest.mark.parametrize("reloaded", [PROVIDER, "sakura.asr", "character_session"])
...
def test_character_switch_rejects_new_recordings_until_session_is_published(runtime):
    application, user_root = runtime
    character = {"characterId": "fixture"}
    boundary = ASRBoundary("asr-binding-test", "credential", user_root=user_root,
        plugin_application_provider=lambda: application,
    ...
    def prepare():
        return boundary.handle({"generationId": "asr-binding-test", "generationCredential": "credential",
            "id": "prepare", "name": "asr.input.prepare",
            "payload": {"recordingId": "recording", "contextId": "draft"}})

    try:
        with boundary.suspend_for_character_change():
            blocked = prepare()
            assert blocked["ok"] is False
            assert blocked["error"]["code"] == "ASR_BUSY"
            assert boundary._tasks == {}
            character["characterId"] = "another-character"
        assert prepare()["ok"] is True
        assert boundary._tasks["recording"].character_id == "another-character"
    finally:
        boundary.close()
