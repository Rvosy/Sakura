...
def test_providers_from_the_same_plugin_retain_independent_data_budgets() -> None:
    def contribute(_request):
        return [
            ContextFragment(
                local_id, "claimed-source", content * 80,
                token_budget=100, provider_id="claimed-provider",
            )
            for local_id, content in (("first", "甲"), ("second", "乙"))
        ]
