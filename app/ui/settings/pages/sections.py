...
class PluginSettingsPage:
    ...
    def _build_detail_panel(
        ...
        owner.plugin_import_button = QPushButton("导入 .plugin", panel)
        owner.plugin_import_button.setObjectName("pluginImportButton")
        owner.plugin_import_button.setToolTip("导入 Sakura 插件包；已存在插件不会被覆盖。")
        owner.plugin_import_button.clicked.connect(owner._import_plugin_archive)
        owner.plugin_export_button = QPushButton("导出 .plugin", panel)
        owner.plugin_export_button.setObjectName("pluginExportButton")
        owner.plugin_export_button.setToolTip("导出当前选中的插件安装目录，不包含运行时数据和模型缓存。")
        owner.plugin_export_button.clicked.connect(owner._export_selected_plugin_archive)
        button_row = QHBoxLayout()
        button_row.setContentsMargins(0, 0, 0, 0)
        button_row.addWidget(owner.plugin_open_settings_button)
        button_row.addWidget(owner.plugin_import_button)
        button_row.addWidget(owner.plugin_export_button)
