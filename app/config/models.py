...
MODEL_SLOT_LABELS = {
    ...
    MODEL_SLOT_VISION_CHAT: "含图聊天模型",
    MODEL_SLOT_VISUAL_CONTEXT: "视觉摘要/OCR 模型",
...
MODEL_SLOT_DESCRIPTIONS = {
    ...
    MODEL_SLOT_VISION_CHAT: "用户消息带图片时直接看原图的模型；留空则继承聊天模型。",
    MODEL_SLOT_VISUAL_CONTEXT: "用于把截图/屏幕转成可检索的视觉摘要；留空则继承含图聊天模型。",
    MODEL_SLOT_MEMORY_CURATION: "用于自动整理长期记忆；留空则继承聊天模型。",
    MODEL_SLOT_THEME_AI: "用于根据角色立绘生成主题配色；留空则继承含图聊天模型。",
}
