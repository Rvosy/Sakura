...
class ToolRegistry:
    ...
    def __init__(self, tools: list[Tool] | None = None, *, logger: Callable[..., object] | None = None) -> None:
    ...
    def execute(self, name: str, arguments: dict[str, Any], *, expected: Tool | None = None) -> ToolExecutionResult:
        """执行一个已注册的工具。"""
        started_at = time.perf_counter()
        tool = expected if expected is not None else self.get(name)
        if tool is None:
            ...
            self._log("ToolRegistry", "工具执行失败", _result_with_elapsed(result, started_at))
            return result
