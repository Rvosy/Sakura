// Shared themed select control for the settings window and its dialogs.
const selects = new WeakMap();
let activeSelect = null;
let nextId = 0;

export function closeSelects(root = document) {
  if (activeSelect && root.contains(activeSelect.select)) activeSelect.close();
}

export function enhanceSelect(select) {
  if (!select || selects.has(select)) return;
  ...
  menu.id = `settings-select-menu-${++nextId}`;
