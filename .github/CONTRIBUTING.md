...
CI 偶发失败遵循 [项目约束](../AGENTS.md#ci-偶发失败必须闭环)。先保留首次失败的 run/job、提交、attempt、
平台和原始错误，再调查根因。复跑只能用于有次数上限的诊断或修复验证，必须保留全部结果；重跑变绿或
后续提交通过不能代替修复。已知失败未闭环时，不建议或执行合并到 `main`、`dev`。
...
- 问题和修改结果；
- 运行过的测试及结果；
- 本轮或相关历史 CI 失败的根因、修复证据及尚未解决的问题（如有）；
- 没有覆盖的风险；
- UI 改动的截图或短录屏。
