...
`MobileChatBridge` 会为每个角色维护轻量运行会话。它使用该角色自己的提示词和聊天历史，但共享桌面端的工具注册表与长期记忆库。每次手机发消息前，它还会从主程序刷新 API Key、模型、上下文提供者和工具配置，所以在电脑设置中更换模型后，下一条手机消息会自动使用新配置。
...
1. 手机端不创建第二个 `MemoryStore` 或 Qdrant 客户端，所以不会和桌面端争抢 `data/memory/qdrant` 的锁。
...
单次上传的请求体上限为 12 MiB。图片会作为 `data:image/...` 数据传给桌面端，再转为模型兼容的 `image_url` 消息；是否能够理解图片取决于当前选择的模型是否支持视觉输入。
