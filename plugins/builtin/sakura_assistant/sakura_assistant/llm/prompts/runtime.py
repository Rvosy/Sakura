...
class ContextPolicy:
    ...
    def select(
        ...
        *,
        ...
        history_source: ContextHistorySource | None = None,
        ...
        dropped_turns: list[ContextTurnDecision] = []
        ...
        if history_source is None:
            ...
            conversations = iter(
                turn for turn in reversed(turns) if turn.category == "conversation"
            )
            observations = iter(
                turn for turn in reversed(turns) if turn.category == "observation"
            )
            minimum_turn_tokens = 0
        else:
            if history_source.minimum_turn_tokens < 1:
                raise ValueError("minimum_turn_tokens must be positive")
            turn_order: dict[str, int] = {}
            conversations = iter(history_source.conversations)
            observations = iter(history_source.observations)
            minimum_turn_tokens = history_source.minimum_turn_tokens

        def select_turn(turn: ContextTurn) -> None:
            nonlocal remaining_total
            if history_source is not None:
                turn_order[turn.turn_id] = turn.sequence
        ...
        def select_from(source: Iterable[ContextTurn], count: int | None = None) -> None:
            iterator = iter(source)
            checked = 0
            # Do not pull the next page merely to discover that no turn can fit.
            # A turn that is too large never ends the search by itself.
            while remaining_total >= minimum_turn_tokens and (count is None or checked < count):
                turn = next(iterator, None)
                if turn is None:
                    break
                select_turn(turn)
                checked += 1

        select_from(conversations, 8)
        select_from(observations, 1)
        ...
        # Projection diagnostics may be produced lazily while pages are read.
        dropped_turns.extend(projected_drops)
        selected_turns.sort(key=lambda decision: turn_order.get(decision.turn_id, -1))
        dropped_turns.sort(key=lambda decision: turn_order.get(decision.turn_id, -1))
