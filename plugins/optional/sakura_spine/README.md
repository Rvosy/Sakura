...
例如将“微笑”改为“开心”，或把 `unique1` 改成便于识别的名称。
导入时保留包内名称；旧包沿用已有名称。修改随草稿保存，重新打开和导出形态后仍保留。
模型会收到自定义名称，但底层继续使用原始皮肤 ID，不影响表情和动画绑定。留空可恢复原名称。

```json
{"skinLabels": {"normal": "平静", "smile": "开心", "unique1": "得意"}}
```

点击表情按钮可预览和编辑其名称；通过“默认表情”选择角色加载时使用的表情。预览其他表情不会改变默认值，
保存、重开和导出均保留所选默认表情，导入时读取包内设置。
编辑器的按钮、滑条和输入框直接沿用工坊的控件样式，并跟随当前主题。

Room 只向模型开放 `skin` 表情选择，待机动画始终循环，默认 1 倍速。播放速度保留在角色工坊的 Spine
编辑页，随草稿保存，不增加程序设置项。表情切换及历史上翻、下翻保持正在播放的循环动画进度和速度；取消一次动作时仍会回到循环动画。
