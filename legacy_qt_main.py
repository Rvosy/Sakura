...
def main() -> int:
    ...
    if instance_status is not InstanceAcquireStatus.ACQUIRED:
        ...
        if instance_status is InstanceAcquireStatus.ALREADY_RUNNING:
            QMessageBox.warning(
                None,
                "Sakura 已在运行",
            ...
            return 0
        if instance_status is InstanceAcquireStatus.FATAL:
...
def _run_acquired_legacy_qt_application() -> int:
    ...
    exit_code = app.exec()
    if not pet_window.resource_manager.wait_for_lingering_qthreads(
        LINGERING_QTHREAD_EXIT_WAIT_MS
    ):
        # A lingering QThread may still own a shared-data writer.  Do not unwind
        # through main's finally and release the mutex while that thread lives;
        # terminate the process so Windows releases the mutex atomically with
        # every remaining thread and handle.
        os._exit(1)
    return exit_code
