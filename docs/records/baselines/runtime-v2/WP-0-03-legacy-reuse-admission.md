...
| R08 | `windows.rs::SecondaryWindowSpec`、`open_secondary_window*` | C190D | 旧次级窗口桥；考察 WP-5-06 | 把设置/Studio/历史/诊断统一塞入窗口模块，提前跨 Phase；D2/D3 | `run_on_main_thread` 的故障经验 | 设置、Studio、历史窗口和强制置顶策略 | WP-1D-01 只用当前 Shell 的最小状态面；完整 diagnostics/repair 窗口后移 WP-5-06 | 中：窗口可触发共享数据写入 | V8；保留主线程创建和焦点失败场景 | WP-5-06 建独立受控 diagnostics 窗口 | 拒绝：不得进入 Runtime v2 |
...
| R21 | `desktop/frontend/runtime-repair/` | C190D | Runtime Repair 页面稿；WP-5-06 | 页面小、无自动修复，可作为后续 Repair 输入；D1 | 基础说明、状态/错误区域 | “Brain”旧术语、仅 refresh、缺少版本/目录/退出 | 基础聊天前只保留 diagnostics 文本/retry/exit；完整页面后移 | 无 Assistant 语义；有误导风险 | V5；缺 Runtime/协议不兼容/路径脱敏/退出 | WP-5-06 用静态 HTML/CSS 重写 | 有条件准入：只复用页面布局和文案结构 |
| R22 | `desktop/frontend/diagnostics/` | C190D | 最小 diagnostics 表现；WP-1D-01 | card renderer 和 raw snapshot 展示有参考价值；D1/D6 | 只读错误文案/状态表现测试 | `host_call`、插件/MCP/TTS/Scheduler 提前展示、私密字段风险 | 最小脱敏 diagnostics 文本、retry、exit | 中：可能泄露路径/配置或制造假 readiness | V5；敏感字段、未知错误、离线页面 | 在当前 Shell 建严格最小状态面 | 有条件准入：只复用 renderer/test idea |
| R23 | `capabilities/default.json`、`tauri.conf.json::app.security` | C190D | WebView 权限/CSP；WP-2-02 | 禁止 shell/fs、限制 remote script 的原则有效；D6 | deny-by-default 思路、`object-src 'none'`、`frame-ancestors 'none'` | 所有窗口共享 capability、dialog 对所有窗口开放、旧 IPC host | 只为 chat.send/chat.cancel 建窗口 allowlist、payload 上限和导航限制 | 中：权限过宽可绕过 Gateway | V4；未知 command、错误窗口、导航和注入 | 从最小 capability/CSP 重新建立 | 有条件准入：只复用安全约束，不复用文件原配置 |
...
| R35 | 双端 `protocol.py` / `ipc.rs` 的 Envelope、`SessionTracker` | C190D | Envelope 错误案例与会话校验；WP-2-01/02 | 类型严格、duplicate ID、sequence gap、stable error 有测试价值；D6 | 错误案例和 strict validation 思路 | `protocol` 单整数、`session_id`、强制连续 sequence、stream kinds | 只冻结最小 Router/chat 所需 generationId、kind/name/deadline 和可选 sequence | 中：错误协议会改变取消和乱序语义 | V4；乱序、旧 generation、未知 kind、缺 capability | 从 ADR-0002 最小 schema 重写 | 有条件准入：只复用错误案例和校验测试 |
...
| R41 | `dto.py::startup_state_dto`、`menu_preferences_dto` | C190D | 旧 startup/Snapshot；考察 WP-1C-02 / WP-2-02 | 混入 `base_dir` 裸路径、设置、模型、插件、MCP、TTS 和 UI layout，没有 revision/generation；D4/D6 | 仅保留“公开角色摘要”字段清单 | 整个 DTO 和 runtime 统计 | 最小 Snapshot 仅 generation/revision/readiness/角色公开摘要/聊天交互摘要 | 高：泄露路径并混合所有权 | V8；保留敏感字段排除和 revision gap 场景 | 按 ADR-0002 从零定义最小 Snapshot | 拒绝：不得进入 Runtime v2 |
...
| R43 | `app/core/assistant_service.py::InteractionSnapshot`、`InteractionHandle`、`_Interaction` | C190D | 最小聊天取消状态输入；WP-2-02 | request/interaction ID、唯一终态和 snapshot 有参考价值；D4/D6 | 状态名、cancel/result/snapshot 语义测试 | `Future.result` 同步等待、单 active 全局 busy、无 generation/deadline | generation-scoped 聊天 identity、幂等终态和 cancel；不先建通用 Operation | 中：取消与完成竞态可能改变用户可见终态 | V4；完成/取消同时、重复取消、Core 重启 | 按 ADR-0002 最小聊天边界重写 | 有条件准入：只复用状态语义和测试 |
...
| R57 | `audio.rs::AudioResourceRegistry`、`capture.rs::CaptureManager` 的资源校验；`screen_observation.py::build_screen_observation_from_private_resource`；对应 observation 路径测试 | C190D | 受控资源描述符经验；WP-4-06 | 路径 containment、大小/mime、TTL、一次读取/删除和 Windows 扩展路径案例有效；D6/D7 | 资源边界测试、opaque ID 和 cleanup 场景 | 裸路径进入跨进程 DTO、session_id 未真正授权、资源类型专用 registry | 由截图这一首个真实资源消费者验证 generation-scoped opaque token | 中：裸路径泄露和跨 generation 访问 | V4；escape、过期、重复读、旧 generation、错误窗口 | WP-4-06 按真实资源建立最窄 registry | 有条件准入：只复用算法和故障案例 |
...
| R60 | `app/core/settings_resource_tasks.py`、`app/ui/settings/resource_tasks.py` | C190D | 资源下载 Operation 候选 | 全局 `_MANAGERS`、daemon thread、同步下载和设置领域混合，不符合 generation/Operation；D4/D7 | task snapshot 字段和取消故障场景 | 全局 manager、daemon thread、模块别名替换 | 后续设置/资源 WP 按真实消费者建立窄任务边界，多个消费者后再抽象 | 中：全局任务可跨 generation 污染状态 | V8；保留重复启动、取消、关闭场景 | 后续按资源类型建立受控任务 | 拒绝：不得进入 Runtime v2 |
...
| WP-3V-01 | 无旧生产文件准入 | 必须组合真实 Adapter/Chat Pipeline、Rust Gateway、恢复和 legacy Qt 回读，Fake Core 不算通过证据 |
...
| R41（拒绝） | Snapshot 裸路径、跨域状态、缺 revision/generation 会污染 UI | WP-2-02 最小 Snapshot 负例；完整 component model 后移 |
...
| R57/R58/R59 | 路径逃逸、过期资源、重复读取、late audio/capture、设备/文件清理 | Phase 4 首个真实资源消费者及组合资源故障矩阵 |
...
$doc = Get-Content 'docs/records/baselines/runtime-v2/WP-0-03-legacy-reuse-admission.md'
...
- `docs/records/baselines/runtime-v2/WP-0-03-legacy-reuse-admission.md`
- `docs/plans/runtime-v2/work-packages.md` 中 WP-0-03 状态和验收记录
