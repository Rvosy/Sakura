...
- `docs/archive/plans/runtime-v2/2026-07-14-tauri-python-core-v2.md`
...
- `docs/records/baselines/runtime-v2/WP-0-01-legacy-qt-baseline.md`
- `docs/records/baselines/runtime-v2/WP-0-02-data-lock-baseline.md`
...
| Assistant Adapter 与 Phase 3 范围 | 主计划要求 Adapter/Facade 复用现有服务；Phase 3 只做基础聊天 | WP-0-01 记录当前能力；WP-0-03 拒绝 BrainHostApplication，条件准入薄 Facade/DTO/测试 | 无 Qt import、readiness、真实 Chat Pipeline 等价性未验证 | 不重写 Assistant；WP-3-01 紧随 WP-1C-04 先验证角色/Session/Provider readiness，聊天、历史、取消和表现随后接入；TTS、截图、设置、Studio 延后 | 需要改业务语义时停止并申请独立范围；可选组件失败只 degraded | 范围一致，无 Phase 4/5/6 偷跑 | WP-3-01 至 WP-3-06 |
