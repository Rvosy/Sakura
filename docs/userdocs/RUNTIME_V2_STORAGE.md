...
可以在“设置 → 系统 → 用户数据”查看并打开数据目录。macOS 更新会替换 `.app`，不会删除 Application Support
中的内容。

TTS 资源默认位于数据目录下的 `tts`，也可以在设置中选择已经存在且可写的外置硬盘目录。更改位置不会自动移动旧文件。

外置盘断开时，Sakura 不会改用默认目录。文字聊天和设置仍可使用，TTS 会显示存储不可用。
