...
Sakura 在 `data/logs/` 下分别保存宿主运行日志、插件日志和模型调用记录（Agent Trace）。
...
同一轮聊天使用短 `op` 标识。模型调用还会带 `trace` 和 `call`，便于在 Agent Trace 中找到对应请求。运行日志记录数量、耗时、状态和原因码。出错时还会保留原始报错、异常原因链、调用栈和子进程诊断，可能包含实际文件路径、本地用户名或失败数据片段。已知凭据和常见凭据格式会脱敏；普通日志不主动记录整段对话、提示词或工具参数，但分享前仍需检查错误详情。
...
插件和关联编号查看。没有记录不代表插件没有运行：插件需要主动接入日志接口，普通 `print`、Python 标准日志及
...
Trace 按完整操作写入。程序在写入途中退出时，下次启动会把残留请求标为 `interrupted`。活动文件接近 32 MiB 时轮转。
