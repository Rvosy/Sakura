...
停止当前生成。系统开启“减少动态效果”时圆环不会旋转，但仍可点击取消。停止生成与回复到达后的
“立即显示”是两个独立操作：前者会取消当前 Core 请求，后者只跳过本地逐字动画。

启动时，角色问候会在桌宠窗口和初始立绘显示完成后逐字出现。模型返回多段回复时，桌宠会按段依次
清屏显示，并在每段开始时同步切换对应立绘；“立即显示”只补全当前段，后面的段落仍会依次播放。
...
使用。若在逐字显示过程中切换，当前段会用新语言从头重新显示，已经播完的段落不会重复。
...
若单次回复失败，可以直接再次发送，应用不会自动重试或重复提交。
