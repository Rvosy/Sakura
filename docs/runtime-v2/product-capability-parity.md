...
只有 `parity-accepted` 或 `approved-replacement` 可以通过 Phase 7 产品等价审查。
...
|---|---|---|---|---|---|
| CAP-001 | 默认启动、单实例、显式 legacy Qt 回退 | Tauri 为默认根；Qt/Tauri 共用锁；安全切换 | WP-1P-02、WP-1P-03、WP-1P-06、WP-3-06 | 可执行/Runtime 定位、锁、退出 | implemented / Windows evidence |
| CAP-002 | 桌宠立绘、气泡、输入、展开状态 | 同一 Tauri App 下连续组合体验 | WP-1P-05、WP-3-03、WP-3-04 | 透明窗口、scale、多屏 | implemented foundation |
| CAP-003 | 点击穿透、拖动、焦点、IME、显示隐藏 | 平台 backend 保持相同用户语义 | WP-1P-05、WP-3-03 | Win32、NSWindow、X11/Wayland | implemented / Windows evidence |
| CAP-004 | 真实聊天、思考、完成与错误 | 无 Qt Assistant Adapter + IPC 垂直链 | WP-3-01、WP-3-02、WP-3-04 | Provider/网络失败不阻塞 Shell | planned |
| CAP-005 | 取消、跳过打字机、请求唯一终态 | Operation cancel 与 UI 表现动作分离 | WP-2-03、WP-3-02、WP-3-03、WP-3-04 | 旧 generation、晚到事件 | planned |
| CAP-006 | 角色、初始消息、立绘/表情切换 | Snapshot + 受控资源 + WebView 状态 | WP-3-03、WP-3-04、WP-5-03 | 资源路径、scale、编码 | planned |
...
| CAP-025 | 诊断、日志、手动修复和安全重试 | Runtime Repair + 平台 diagnostics | WP-1D-01、WP-1D-02、WP-1D-03、WP-5-06 | 路径、日志、权限 | planned |
...
| CAP-030 | 用户数据与 Qt 双向兼容 | Qt -> Tauri -> Qt 全量读取/允许写入门禁 | WP-3-06、WP-7-03 | 路径、锁、原子替换、编码 | planned |
