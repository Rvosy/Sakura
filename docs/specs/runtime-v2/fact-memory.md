...
可选插件 `fact_memory`（版本 `0.1.0`）使用 Plugin API v4，在自己的设置窗口提供便签 Collection，
...
关键词按英文或中文逗号、顿号、分号、换行分隔；任一关键词在输入中命中即可入选，匹配忽略大小写。
...
只读工具 `fact_memory_search` 与 Context 共用相同召回和预算规则：
...
插件聚焦测试覆盖角色分区、字段边界、关键词与无关键词分支、条数/正文预算，以及无效写入保留已有数据。
