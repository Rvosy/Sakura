...
桌面角色与工坊预览使用 2 倍超采样：画布按实际显示尺寸、屏幕像素密度和 2 倍倍率绘制，
再缩小到显示尺寸，减轻透明边缘和细线的锯齿。布局与交互坐标不随倍率改变，原始素材不改写。
画布仍受 GPU 尺寸限制和 16,777,216 像素总量上限约束，超过上限时等比降低渲染尺寸。
...
由 `normal` 提供正常五官；不会把多个表情叠加。

`skinLabels` 是可选的皮肤 ID 到显示名称的映射。键必须存在于骨骼中，值必须是最多 120 字符的字符串；
允许重复名称。显示名称完全来自形态资源；未设置或留空时显示原始皮肤 ID，插件不内置表情名称映射。
动画显示骨骼内的原始动画 ID，不由编辑器翻译。
