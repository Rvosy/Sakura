...
选择又回到已提交角色时清除角色 dirty 状态，不写配置、不读取 lifecycle，也不重启。暂存目标期间仍只展示和
  ...
  所属角色误判，暂存期间锁定外观、语音和 Memory 页面，并暂停所有插件 Collection 的查询和写入；角色下拉和普通全局字段仍可编辑。
...
- 当前角色有未保存的外观、语音、Memory 或任一插件 Collection 新建/编辑草稿时，选择另一角色必须恢复到前一个下拉值并提示先
  保存或放弃。Collection 草稿保护不受 surface 或插件业务类型影响。下拉草稿本身可反复改选；选回已提交角色即取消待切换状态。
...
- 只有后端返回已提交的 restart receipt 后才进入 switching。草稿检测覆盖 Memory 原生编辑器与所有插件 Collection editor；
  待应用与切换期间暂停所有 Collection 查询和写入。在途查询不得回填；异步删除确认返回后复核编辑器与切换状态。
  本地切换与外部重绑定各自完成后才释放其操作锁，过期 generation 或普通目录通知不能提前解除另一操作的锁。
  取消选择或切换结束后恢复被暂停的当前查询，不重放写入。
- 收到实际角色变化的已提交 restart receipt 后进入 switching，立即清空并隐藏旧角色 Memory 列表、编辑器、所有插件
  Collection 页面状态和历史内容，关闭 editor portal 并失效在途查询；切换期间禁用角色导入/选择及角色相关操作，不整页 reload。
  外部角色变更也执行相同清理。以 Collection 原属角色和实际角色判断变化，目录刷新和同角色 generation 更新不应误清或漏清。
- 当前角色有未保存的外观、语音、Memory 或插件 Collection 改动时禁止导入语音。导出仍可读取已保存
  的角色包；有待应用的角色切换时，导入语音和导出都保持禁用。
- Provider、Tools、Plugin 和 Screen Awareness 等全局设置草稿保留，并在新 generation 就绪后重新绑定；
  外观、语音、Memory 和插件 Collection 草稿不得迁移到另一角色。同角色 Core 重启保留仍存在 Collection 的草稿和筛选，
  将其绑定新请求状态，旧查询、错误和写入回执不能覆盖新状态，也不得自动重放旧写入。
