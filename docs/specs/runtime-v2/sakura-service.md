...
- `sakura.cialloo.cn` 继续承载产品站；`hub.sakura.cialloo.cn`、`dl.sakura.cialloo.cn` 仅预留命名，尚未启用。
  官网源码位于 `services/sakura/site/`，独立构建和发布。官网静态文件同步必须保留同一站点根目录中的
  `service/` 公开清单与服务器隐藏配置，不能覆盖控制台发布的在线状态。
...
相关决策见 [ADR-0055](../../adr/0055-domestic-updater-manifest.md)，维护入口见
