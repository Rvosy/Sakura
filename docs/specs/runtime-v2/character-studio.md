...
- `open_character_studio({ characterId, resourceId? })` 创建或聚焦唯一 `studio` 窗口。窗口初始化完成前保持隐藏。传入 `resourceId` 时打开“形态”页并选中对应形态；已有工坊窗口通过导航事件接收目标，切换角色前保存当前草稿。目标形态已移除时明确报错。
...
导出对话框默认使用当前显示的形态名称作为文件名，保留中文；文件名非法字符替换为下划线，Windows 保留名称加下划线前缀，不使用内部资源 ID。
组件导入生成新资源 ID，仅加入目标草稿，保留已有默认形态；目标原本没有形态时，首次导入的形态成为默认。
导入保留其人格、voice、角色 ID 与已安装包；可以放弃草稿撤销。详见表现插件合同的文件交换部分。
...
发布前校验草稿，并与已安装包逐文件比较实际内容。比较包括模型、立绘和未知资源，大文件按块
检查取消，不单凭文件大小或修改时间判断相同。两者完全一致时只把草稿标记为 clean，不复制 staging、
不新增备份，也不停止当前 generation 或触发 Core 重建。首次保存旧包时，manifest 的规范化仍可能产生变化。

当前角色内容变化时，只暂存和替换变化的文件，未修改的资源保持原位，空目录不视为内容变化。发布顺序为：

```text
完整校验
-> 将变化文件复制到 characters/.studio-transactions/<uuid>/staging
-> 将旧角色完整复制到 rollback
-> 语音资源变化时暂停活动语音服务及其硬依赖方
-> 写 publish-journal.json
-> 替换或删除变化文件，最后替换 character.json
...
新角色和非当前角色继续使用完整 staging 与目录改名事务。当前角色 journal 额外记录 `changed_files`；
发生失败或进程中断时，从 rollback 或完整 backup 恢复这些文件，不移动未修改资源。新增文件在恢复时删除，
已编辑草稿保留。正常失败在语音服务恢复前完成回滚；恢复再次中断时保留 journal 供下次启动继续处理。
...
删除 journal 是提交点；此后的运行态更新失败不能回滚已经发布的文件。旧版目录事务 journal 仍可恢复。
...
增加空间。当前角色只暂存变化文件，同时保留旧包完整备份；非当前角色仍暂存完整包。
空间按保留版本的实际大小计算，不随成功保存次数持续增长。
...
发布后立即发送 `sakura://character-catalog-changed`，设置页重新读取角色列表，不重绑定全局设置控制器。
工坊关闭本身不触发目录刷新。保存前冻结旧表现编辑器并保留静态内容；收到保存回执后刷新表现插件目录，
待新编辑器就绪后替换，不等待重启事件。运行态更新失败时保留已保存的数据并显示错误。
...
其中保存回归使用正式桌面 CSP 和真实有界 Core 路由，覆盖多图预览及局部发布后的编辑器挂载顺序。
