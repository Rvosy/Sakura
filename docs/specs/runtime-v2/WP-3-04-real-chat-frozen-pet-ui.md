...
- 输入栏顶部固定在气泡下方，所有内容增高只向下发生；输入变化不得移动气泡。子内容、面板矩形和原生
  ...
  WebView 内容和平台输入玻璃必须使用相同 staging 和曲线，结束时不得留下 CSS 几何尾帧。扩展的原生
  Composition 动画必须在 `apply_pet_layout` 内直接开始，不得等待 layout ack；只有收缩在确认后由 WebView
  显式启动对应 revision。禁止使用固定延迟推测 ack 耗时。收缩时精确窗口裁剪必须覆盖完整位移动画，
