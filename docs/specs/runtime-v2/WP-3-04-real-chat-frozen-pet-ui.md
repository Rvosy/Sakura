...
- 输入栏顶部固定在气泡下方，所有内容增高只向下发生；输入变化不得移动气泡。子内容、面板矩形和原生
  ...
  从 52px 胶囊进入至少两行文字时，外框和玻璃先在首帧同步抬高到“仅容纳当前文字、尚未加入工具栏”的
  staging 高度，再以 260ms 插值打开到最终工具栏布局；收回不经过 staging，直接平滑收至 52px 胶囊。
  WebView 内容和平台输入玻璃必须使用相同 staging 和曲线，结束时不得留下 CSS 几何尾帧。Windows 原生
  layout ack 只能准备玻璃终点并保持起始 clip；必须由 WebView 创建外框 WAAPI 的同一帧显式启动对应
  revision 的 Composition 动画，禁止使用固定延迟推测 ack 耗时。收缩时精确窗口裁剪必须覆盖完整位移动画，
