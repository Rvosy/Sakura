...
- info 保持单行；带详情的条目以摘要行本身作为唯一展开入口，不再附加单独的“查看详情”行。warning 可展开；
  error 默认展开并优先显示诊断、错误代码、原因代码、阶段、错误类型、根因类型、代码位置、问题编号、状态、
  耗时和关联编号。info 有安全详情时保持折叠，可按需展开；相邻完全相同记录合并为 `×N`。
