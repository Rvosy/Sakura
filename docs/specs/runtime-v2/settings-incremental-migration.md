...
`chat.subtitle_language` 持久化 `subtitle_language: "zh" | "ja"`，以及 `show_japanese_original`。
`subtitle_language` 缺失或非法值读取为默认 `zh`，下一次成功保存时规范化；`zh` 优先展示 segment
`translation`，空值回退 `text`，`ja` 展示 `text`。`show_japanese_original` 缺失时为关闭。开启且当前为
`zh` 时，气泡在中文下方用较小字号再显示不同的 `text`；关闭、语言为 `ja`，或两行文本相同，都只保留原来的一行。
主窗口通过 `sakura.chat.subtitle.toggle` 和 `sakura.chat.japanese-original.toggle` 右键菜单动作切换，
菜单 manifest 必须返回 checked 状态；保存失败保持旧文件、旧运行值与旧勾选态。字幕语言切换成功后，
输入中的当前段立即清空并按新语言从头重播；settled 或当前会话回看段立即完整替换，不等待下一条回复、
不回放已完成段，也不切换立绘。日文原文开关只增减下方原文，不重播当前段。
