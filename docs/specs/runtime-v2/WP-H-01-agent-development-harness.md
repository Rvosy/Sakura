...
- 所有路径使用仓库相对 `/` 分隔形式，不允许绝对路径、`..`、空模式或反斜杠；精确重复失败。
- allowed、forbidden、protected 之间出现相同模式或可证明的父子覆盖冲突时失败；无法静态证明的 glob
  在实际 changed set 上按“禁止/受保护优先”求值。
- 文档必须存在且位于声明职责目录；profile 必须在 `suites.json` 注册；`base_ref` 必须由
  `git rev-parse --verify <ref>^{commit}` 解析。
...
检查顺序为 protected/forbidden、allowed、依赖、关键测试删除、冻结契约。`data/**`、`characters/**`、
`third_party/**` 默认受保护，但具体 Harness 任务仍必须在契约中显式列出。Harness 自身不是永久禁止项；
修改它的任务必须显式允许，同时当前任务契约及其引用的 Spec/ADR/验收条件在实现 diff 中变化即失败并
要求独立重新预检。
...
- `preflight <ID>`：契约、状态、依赖、文档、profile、base ref、路径规则和开始工作树检查。
- `check <ID>`：快速运行冻结、scope、protected 和 dependency 检查。
...
`3` 自动门通过但必需人工验收仍 pending。`verify` 前置失败后不得运行昂贵 profile。
...
CI 在 `refactor/tauri-runtime-v2` push 时增量增加 Harness 自测、docs、smoke、schema 与 scope 测试；现有
三平台准备、Xvfb/WebKit、Qt cleanup 和 platform matrix 保留。任务级 verify 证明任务契约，本来就存在的
长三平台 workflow 继续证明平台/设备层，两者互不冒充。
