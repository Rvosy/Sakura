...
屏幕感知插件在 `prompts.py` 持有完整搭话规则：先结合近期对话与画面变化判断进展，再用角色语气回应具体内容，
按工作、学习、娱乐或社交场景调整表达，不重复建议、不机械提醒休息、不臆测看不清的信息，也不暴露插件实现。
角色态度、情绪和分段随内容变化；补充资料仅使用实际提供的只读低风险工具，已有图片不再次截图。
