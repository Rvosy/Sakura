...
| 正在准备 | 底部工具行中央显示「正在准备」，右侧加载状态，左侧可取消；禁止发送 | 引擎可用且麦克风成功打开后才进入录音 |
| 正在录音 | 左侧取消，中间真实音量波形，原麦克风位置变为停止按钮；发送禁用 | 点击停止后识别，取消则丢弃本次录音 |
| 正在识别 | 底部工具行中央显示「正在识别」，右侧加载状态，左侧可取消；禁止发送 | 成功回填，失败提示，或用户取消 |

录音布局参考用户提供的横向波形条：安静时是短线或圆点，有声音时出现上下对称的圆角竖条。准备、录音和识别
都占用最下方的工具行，状态或波形位于取消与停止按钮之间。多行输入框保持原有高度，上方草稿继续显示但不可
编辑；单行输入框暂时隐藏文字，为语音状态留出空间。附件暂时隐藏，已有草稿和附件仍保存在原会话中。
完成或退出后恢复编辑、附件与焦点。准备和识别不在输入栏外显示独立提示框。
...
- 切换会话或角色、关闭或隐藏整个桌宠窗口、重建 Core generation 时取消任务并失效回填资格。拖动桌宠时
  仅暂时隐藏输入栏，准备、录音和识别继续，松开后恢复当前语音状态。其他窗口短暂获得焦点不等于关闭录音；
  系统权限弹窗不能导致录音启动后立刻被错误取消。
...
新录音可立即打断完成动画。工具行的状态文字和真实波形淡入淡出；语音输入期间草稿不可编辑或获得焦点，
...
采集端快速跟随音量上升与停顿，绘制端合并短时间窗的峰值，拉开安静、轻声与正常说话的高度差，避免多次
平滑抹平音节起伏。采集速率不受动画速度影响，不能使用随机或预录的模拟包络替代真实音量。
...
Hub 在调用 Provider 前使用公共 `context.bind(serviceKey)` 固定进程，再核对登记时的 Provider scope。
该任务的就绪查询、受理、`poll/cancel` 共用绑定代理；不能先单独核对 scope，再通过动态代理发送旧任务调用。
提供者退出或同 ID 重载后，旧绑定失败，旧 job 不得交给新进程，即使新进程复用了相同 `jobId`。

Core 在开始准备语音输入时固定 `sakura.asr` 的 Hub 实例，任务后续调用继续使用该绑定。
成功识别返回后，核对 Hub 与 Provider 仍属于原 scope，才允许结果进入待回填状态；
首次向 UI 交付文本前再次核对两者。成功暂存后才发生的重载也不得把旧文本交给当前草稿。
任一实例失效时，输入任务报告 `ASR_PROVIDER_UNAVAILABLE`；已经取消的任务保持取消，迟到结果不得恢复成功。
下一次用户发起的输入可以绑定新实例，旧任务不重新绑定、重试或转交其他引擎。
这些调用复用 Runtime 的普通服务绑定；Provider 登记身份检查、音频授权和读取租约仍由原入口负责。

同 Core 切换角色时保留可复用引擎，并在整个角色切换作用域暂停新录音请求（`ASR_BUSY`）。
进入该作用域先取消原任务；即使 A→B→A 后 Hub、Provider 的进程身份不变，原任务的迟到成功或尚未交付结果也不得写入新会话。
切换作用域结束后，新输入重新受理，不恢复或重放原任务。
...
- 用事件或屏障固定 Provider/Hub 重载与 `poll/cancel` 的先后，验证旧 `jobId` 不会发给新进程、返回后已失效的结果不回填；
  成功暂存后、首次 UI poll 前任一实例被重载，同样不得交付文本。
  同时检查取消后的录音读取租约回收，不以正常完成一次识别代替重载竞态验证。
...
2026-09-14 的实例绑定修正增加 6 个真实进程回归。它们在修复前分别复现 scope 检查后的旧调用误发、
识别返回后来源失效，以及成功暂存到首次交付之间来源失效；修复后均通过，且没有交付旧文本或泄漏录音资源。
最终 `journey-asr` 通过 3/3：Python 45 项、Rust 11 项、前端 22 项通过，Rust 的 1 项真实麦克风测试未运行。
本轮使用隔离 WAV、真实插件进程和音频租约，未运行真实麦克风、识别模型或原生窗口人工验收。
