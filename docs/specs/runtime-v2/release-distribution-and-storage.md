...
ID 已被其他可访问角色占用，则为遗留副本分配带序号的新 ID，清单写回保留备份。导入和启动读取都保留包内主题
颜色，`theme.source` 仅为来源元数据，统一解析为当前 `package`，不得因旧版内部来源标记拒绝角色。启动读取不
...
四个语音与记忆插件会延迟加载部分依赖，单独导入插件入口不能证明依赖可用。迁移复查和候选准备在同一隔离进程中导入 `migration_sources.json` 的 `runtimeImports`，发行升级验收也读取这份列表；不启动模型、不联网，不把目录中存在任意文件视为依赖完整。检查失败继续选择其他本地来源；所有来源均不可用时保留未完成状态及原始异常。

迁移同时复用 Inventory 对已声明渲染器、编辑器等能力的检查结果。Spine 渲染器等文件缺失时，不能仅因 Python 入口可导入而记录完成；诊断保留具体能力与原因。
...
桌面宿主首次观察到当前核心正在迁移时，立即打开设置窗口显示迁移进度，不等待桌宠资源或 Assistant 初始化。每次核心启动只自动打开一次，关闭窗口后不因进度更新反复弹出；已过期的核心通知不打开窗口。设置页先显示“正在迁移插件”、当前插件及完成数量，暂缓读取插件业务设置；迁移结束并且插件启动状态稳定后自动进入正常设置。成功提示随加载界面收起，失败时保留重启核心入口。普通启动不额外打开迁移窗口。
...
实际迁移通过现有日志桥记录 `plugin.migration.*` 事件，包括开始、逐项结果、来源选择、候选不可用、恢复原目录和最终汇总。记录携带插件 ID、代码与依赖来源类别、版本、阶段、结果和耗时；失败保留稳定错误码、原始异常链与栈。候选不可用只记过程信息，最终失败才生成错误报告。已完成且无需处理的启动不重复记录迁移过程。远程投影和开关见[远程诊断与运行统计](remote-diagnostics-telemetry.md)。
