...
现有平台桌面 MCP server；不得以测试专用 manager 代替其中任一条产品路径。本 WP 不修改
`tools/mcp/**`，不新增 MCP server，也不迁移 Python 插件、TTS、截图 resource token、浏览器、主动调度、
提醒或通用 worker 平台。
...
- 设置窗口只开放现有平台桌面 MCP 开关与脱敏运行状态。DTO 至少区分平台支持性、持久化偏好、当前
  generation、配置有效性以及 server 的 `disabled|starting|ready|degraded|stopping|stopped` 状态和稳定
  reason code；不得暴露 command、args、env、headers、URL 凭据或工具参数。
...
Windows 实机验收使用隔离 assistant root 启用桌面 MCP，确认 server ready、工具可见、原生确认、成功调用、
拒绝/超时、Core 重建、设置状态恢复和退出零残留；扫描日志与 WebView 数据，确认 command、args、env、
headers、凭据、工具参数/结果和绝对路径零泄漏。项目负责人明确验收前不得标记 `accepted`。
