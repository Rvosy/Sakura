...
`uv` 依赖下载任务时将当时的系统代理传入该子进程；GPT-SoVITS 插件启动 macOS 安装脚本时也传入当时的系统代理。
运行中的安装任务保留启动时的配置。

可选插件资源下载不读取 Shell 的 `settings.download_sources`。SenseVoice VAD 与 GPT-SoVITS macOS Miniforge
按插件中的固定镜像顺序下载；GPT-SoVITS 源码仅使用支持 Git fetch 的镜像，然后回退官方地址。具体来源与覆盖入口见
[用户指南：下载插件资源](../../userdocs/RUNTIME_V2_PLUGINS.md#下载插件资源)，固定版本由各插件资源声明与安装脚本维护。
来源切换保留失败原因；包大小或格式不符、磁盘写入失败和用户取消不触发重新下载。Git fetch 失败最多回退官方一次，checkout 与后续安装失败直接结束。
GPT-SoVITS 脚本失败通过现有 diagnostics 服务提交 `tts.bundle.install.failed`，保留退出码和最多 4000 字符的完整输出行；
超长单行整体省略，避免截断凭据标签后泄漏内容。宿主按现有日志和遥测契约脱敏，插件不直接联网发送诊断。
macOS 安装脚本使用独立进程组；取消或关闭插件时回收整个组，静默输出及父脚本先退出不阻塞取消，也不留下继续下载的后台进程。
...
市场搜索栏提供显眼的“上传插件”入口，点击后打开说明弹窗：作者先将源码上传到自己的公开 GitHub 仓库，再提交插件收录 Issue。
弹窗的“打开 Issue 页面”通过系统浏览器打开 Sakura-Registry 的 `submit-plugin.yml` 投稿表单，由作者填写插件 ID、仓库地址和版本信息并提交。
...
市场卡片和详情只展示用户可理解的功能领域，不重复显示服务、工具等实现分类。详情按内容高度展开，
名称下显示作者与领域；简介只出现一次，正文展示对应版本仓库的 README，保留作者的功能说明、使用步骤、
资源要求和许可证信息。项目主页与原文可在系统浏览器打开。说明异步加载，失败可单独重试，不阻断安装或管理。
README 由 Shell 根据已加载目录中的 GitHub 仓库和固定 commit 获取，使用现有下载源，限制为 256 KiB、45 秒。
省略重复的文档主标题，包含插件 ID 或 Plugin API 的资料表默认折叠，使用步骤与资源要求正常展开。
说明不接受前端任意下载地址；Markdown 的原始 HTML 按文本显示，只允许 HTTPS 链接，图片以链接显示。
说明中的版本文字属于作者原文，不覆盖目录与本地运行状态；独立正文、使用影响和更新说明有内容才显示。
推荐安装版本有更新说明时展示“最近更新”；其余有说明或撤回、不兼容原因的版本收进“历史版本”。
空版本记录不占位。插件 ID、接口版本和已知下载大小收进“技术信息”，刷新目录和安装进度不折叠已展开的资料。
底部合并安装状态与本地版本；未安装时显示安装目标版本，更新按钮显示目标版本，不另列“推荐版本”和“本地状态”。
不兼容、内置插件随应用更新等原因放在操作附近；启用中的插件可直接更新，操作前提示会短暂停止并恢复。
已安装插件即使无法更新或没有兼容目录版本，也保留管理入口。
分类、简介与资料通过留白和浅底色分组，不在卡片、资料与版本记录之间重复添加分割线。
市场目录选择、安装门禁和详情交互由 Harness `journey-plugin-marketplace` 验证；浏览器检查使用隔离目录和宿主快照，不安装真实插件。
...
全部下载源失败时，错误保留各源的 HTTP 状态或底层网络原因，省略请求 URL，沿用现有日志与遥测脱敏边界。
...
没有可读清单时，仅允许根据目标插件 ID 修复其规范目录，拒绝符号链接和 reparse point；多个同 ID 副本冲突时先由用户清理冲突。
迁移失败留下的孤立依赖根在显式安装中经过 staging 重建；不凭 marker 和可导入入口复用可能缺少延迟加载模块的环境。
下载完成后，Core 在同一次生命周期操作中停止目标和正在运行的硬依赖消费者，再替换代码与依赖。
新版本恢复原启用状态，原先运行中的依赖消费者按依赖顺序恢复；原先停用的插件保持停用，不重启无关插件。
当前互动尚未结束时返回可重试错误，不在对话执行中替换插件。更新复用现有代码、依赖备份和配置恢复流程，
保留插件配置与数据；包校验、运行时登记、新版本启动或依赖消费者恢复失败时，恢复旧版及原运行状态。
旧版恢复失败必须单独报错，不能把仅写入新文件当成更新成功。若操作前插件已经损坏，回滚恢复原文件和状态，不把原本无法启动再次误报成回滚失败。
未保存的设置草稿需先保存或还原。完整来源持久化和固定版本更新策略不在本次实现范围。

市场目录和项目 README 保存在用户根目录的 `data/cache/plugin-marketplace/`，跨设置窗口和应用重启复用。
打开市场先显示缓存目录，再按下载源配置后台刷新；刷新过程保留列表、筛选、详情和滚动位置，失败时保留缓存并提供重试。
兼容性仍使用当前 Core 的 API 与服务，安装状态来自当前插件清单，不持久化运行状态。首次没有缓存时才显示下载等待状态。
成功响应以原子替换写入缓存；缓存缺失、损坏或格式不受支持时重新下载，写缓存失败不影响本次浏览。
README 按 GitHub 仓库及固定 commit 缓存，命中后不重复下载；目录返回时同时读取已有说明，缺失说明以最多两个并发请求提前加载。
新版说明未就绪时保留可用旧版正文并标注版本，失败后仍可阅读和重试。旧窗口或旧请求的返回不得覆盖当前界面。
