...
- 淡入前先提交包含目标组件的原生 region，再显示 WebView 元素。气泡淡出时先播放 220ms WebView
  ...
  不含气泡的 region。动画事件缺失时使用 320ms 超时，所有系统动态效果设置下都保留动画等待与两个绘制帧。
...
- `bubbleMaxHeight` 持久化字段继续兼容既有配置。默认固定模式把它解释为对话框的精确高度；用户开启
  ...
  系统减少动态效果设置不改变过渡时长或原生/WebView 的同步方式。输入框仍可按输入行数在契约范围内自适应。设置页拖动
...
Sakura 的动效由应用自身控制，不读取 Windows、macOS 或 Linux 的“减少动态效果”偏好，也不通过
`prefers-reduced-motion` 禁用、缩短或降频。该策略覆盖桌宠、输入栏、图标、语音波形、逐字字幕、等待提示、
设置与插件界面、首次启动引导、历史记录和日志查看器。Morphicons 显式使用 `reducedMotion: "never"`。

相同状态不重复播放；取消、失败、角色切换和销毁必须回收旧动画与回调。首帧初始化、无实际几何变化及
不支持动画 API 的现有功能回退仍可立即提交。业务动作不能等待装饰动画完成，真实波形不得用模拟数据替代。
