...
- 桌宠、设置和角色工坊共用主题提示浮层。静态和动态提示使用 `data-tooltip`，不使用原生 `title`；
  自绘下拉框透传原控件和选项的提示。提示随主题更新，靠近窗口边缘时调整位置，避免被滚动容器裁切。
- 桌宠通过 `set_pet_tool_dock_surface` 的可选 `tooltipRect` 同步浮层原生可见与命中区域；坐标使用
  WebView 视口 CSS 像素，只换算系统 DPI。提示与工具面板同时出现时保留各自区域，收起提示不移除工具面板。
