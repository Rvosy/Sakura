...
- 系统托盘菜单继续使用 Tauri 原生菜单。桌宠菜单只允许执行显示/隐藏、设置和退出；中文字幕、完整访问
  权限、置顶、历史与运行日志仅以灰色禁用项表达迁移状态，不发送命令，也不作为对应能力已完成的证据。
- 自绘菜单打开期间 Rust 临时恢复整窗命中区域，关闭后按最新布局、DPI、立绘缩放和 alpha mask 恢复
  精确区域；恢复失败继续采用整窗可交互的安全回退，不能留下不可点击窗口。
- CAP-022 保持 `planned`，后续仍由 WP-5-04 补齐置顶、快捷键、开机启动和三平台完整桌面能力。
