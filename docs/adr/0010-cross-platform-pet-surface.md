...
- macOS 只在缩放手势活跃期间临时使用“当前控件布局与 150% 立绘”的稳定包络；静止态必须收紧到
  ...
  包络与精确命中。macOS 不扩展到 Windows 的全部合法控件布局极值；Windows 的常驻 HWND 路径不受
  该临时包络策略影响。
- Linux 采用与 macOS 相同的缩放状态生命周期，但不复用 AppKit API：静止态使用当前倍率立绘 alpha
  与当前可见控件的真实并集；手势开始一次扩到“当前控件 + 150% 立绘”，刻度只更新 WebView 合成
  transform，并以 latest-wins 单槽队列追赶当前倍率精确 GTK input region；手势结束以新 revision
  清空旧帧并一次收紧到真实并集。X11/XWayland 通过现有 GDK `move_resize` 在一个配置请求中提交位置
...
- Linux 在未显式指定 `GDK_BACKEND` 且存在 `DISPLAY` 时优先 X11/XWayland，以获得完整全局定位和
  单次 move+resize；native Wayland 保留精确 input region 与手势首尾各至多一次 resize，但明确标记
  全局锚点降级。
...
整体回退 schema v5、surface transaction 和三平台 backend，恢复固定窗口候选；回退不修改用户配置、
