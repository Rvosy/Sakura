...
- 0.9.x TTS 配置只在显式导入时，由 `app/legacy_import/configuration.py` 转换为 Provider 插件配置，
  再由 `importer.py` 调用各插件的 `_parse_config` 校验。普通启动不读取旧 `api.yaml.tts`；Core、Rust 和
  TTS Hub 不读取 Provider 私有字段，也不实例化旧 Provider Registry 或合成服务。
...
生命周期边界可由单元测试证明，且远程参考音频失败是显式的。旧版导入需要维护配置转换，远程服务运营者需要
