...
可选表现解析也不进入 Core 的正文提交路径。Core 保存绑定身份与原始控制载荷，桌面在现有段落准备点
请求解析，并与语音合成并行。解析成功后仍在播放开始时派发；失败保留原画面、字幕和语音。取消不等待
解析返回，旧绑定和旧回复的迟到结果失效。这复用现有播放顺序与绑定身份，不另设表现任务队列。

## 考虑过的方案

- 引入完整 Agent 框架：会同时引入会话、历史、工具与任务状态，重复现有产品责任；采用方案只复用模型 SDK。
