...
```text
当前状态：以 docs/plans/runtime-v2/work-packages.md 第 2 节为唯一真相源
...
2026-09-12 起，本文原有固定 portrait/portraitChoices 投影与宿主图片选择由[表现插件合同](../../../specs/runtime-v2/visual-plugin-boundary.md)取代。
...
`docs/specs/runtime-v2/settings-incremental-migration.md` 逐 feature 迁移；这项排期调整不扩大本 WP 的生产
范围，也不绕过其尚未关闭的三平台硬门。
