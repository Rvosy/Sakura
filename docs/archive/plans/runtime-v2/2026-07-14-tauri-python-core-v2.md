...
> 执行阶段、Work Package 状态和当前启动点：仅见 `docs/plans/runtime-v2/work-packages.md`
...
- 完整回复返回后的打字机展示。
- 思考、错误、降级和重连状态反馈。
- 桌宠右键菜单，以及同一 Tauri App 内由菜单打开的普通设置窗口。
...
产品桌宠只使用一个固定组合布局；聊天与生命周期状态不得再映射为不同的窗口几何：
...
立绘拥有固定桌面锚点，气泡和输入框拥有固定相对位置。长回复只能在气泡内部滚动，不能通过
切换 idle/bubble/composer/expanded、改变原生窗口包络或移动立绘处理。产品界面不保留“聊”、主题、
Fake restart、visibility probe 等常驻功能切换栏；诊断动作只能存在于默认隐藏的 debug-only 验收面板。

正常视觉验收必须使用 `characters/**` 中的真实角色包。自绘测试 SVG 只能作为资源加载失败的
fallback，不能作为默认画面、截图基线或 Work Package 接受证据。
...
- 设置页沿用现有 Tauri 前端资产并迁入同一 Tauri App；迁移时建立单一规范源，不能长期维护
  `desktop/` 与 `tools/settings-tauri/` 两份会漂移的手工副本。工作室迁移前再决定长期前端框架。
...
- `docs/plans/runtime-v2/delivery-governance.md`
...
- [ ] 历史技术门验证 idle/bubble/composer/expanded 和固定立绘锚点；Phase 3 产品 UI 改为固定单一组合布局后，旧状态只保留为历史证据。
...
### Phase 3：产品 UI 外壳与基础聊天垂直链

- [ ] 从现有 Python 服务适配角色、Chat Pipeline 和基础 Provider。
- [ ] 在固定产品 UI 中显示当前真实角色的立绘、名称、主题和初始消息。
...
- 气泡内容变化不移动立绘桌面锚点。
- 立绘切换无明显白闪、尺寸跳变或布局抖动。
- 长文本在气泡内部有界滚动，不改变原生窗口包络。
- 中文输入法候选框位置正确。
- 125%/150% DPI 下文本和立绘清晰。
- Core 重启状态与正常气泡使用一致的视觉语言。
- CSS 动画不能阻塞输入、取消或关闭。
- 正常产品画面不显示 Fake Core 标签、几何读数或验收控制条。
- 设置窗口重复打开只聚焦已有实例，关闭设置窗口不退出桌宠或 Core。
...
- `docs/specs/runtime-v2/product-capability-parity.md` 的发布必备行全部达到 `parity-accepted` 或获批替代。
...
22. 设置窗口宿主在 Phase 3 提前迁入同一 Tauri App；从 WP-3U-02 起按 `docs/specs/runtime-v2/settings-incremental-migration.md` 逐 feature 交付，领域设置随对应能力 WP 开放，Phase 5 只做缺口、关闭清单和首次设置收口。
