...
加入角色与显示方式选择，用可操作的外壳判断普通用户能否看懂。交互方案确认后，再启动立绘拆分。
另有[角色工坊 demo](../../desktop/frontend/prototypes/studio-demo/README.md)，用于评审同一角色添加多套立绘、
由插件提供 Live2D/3D 编辑入口，以及保存后在设置中选择形态的流程。
...
新增交互为“角色 → 显示方式 → 对应插件设置”。只有一套立绘时不增加多余选项；
下拉框只列出当前角色包实际包含的形态，首次打开时选中包内默认形态，不单设“跟随角色包”选项。
插件私有参数仍在插件设置里。缺少插件时才展示原因和处理入口。
大小滑条统一命名为“角色大小”，不随立绘、Live2D 或 3D 类型改变。
用户的表现选择按角色分别记录，切换表现不更换人设或自动切换语音引擎。

评审时重点看三件事：只用立绘是否仍然简单，多种表现能否找到并切换，缺少插件是否知道下一步怎么做。
控件是否需要收起仍可据此调整，不作为已经接受的产品合同。
demo 刷新页面会恢复模拟数据；不会读取真实配置、安装插件、下载模型或执行角色归档。
工坊 demo 复用正式工坊的全部页面和控制器，在“角色形态”中添加、命名、编辑、移除资源和指定默认形态。
可用插件决定添加类型；停用插件保留资源并锁定对应编辑区。草稿保存在页面内存中，角色保存后同步到设置预览。
工坊 demo 只验证交互，不提前定义多表现数据协议；真实渲染、原生窗口与后端兼容性仍按后续切片验收。
