...
插件生态的后续顺序见[开放插件生态总计划](open-plugin-ecosystem.md)：M1 已修正 Context 分类，M2 已打通执行器替换，
下一主线 M3 迁出默认模型，随后 M4 迁出对话策略，再按真实消费者完善协作和界面。
该计划不改变下表尚未核对的历史验收状态。
...
- Plugin Runtime v4 已完成 v4-only 切换和完整验收：官方默认实现可替换，每插件独立进程和 dependency
  ...
  这里的默认实现指已迁入插件的能力；M2 已让正常聊天入口选择独立执行服务，默认模型客户端、Agent 和管线仍由本地适配器包装，
  完整迁移属于尚未实施的 M3/M4。
- 开放插件生态 M1 已提交 `2097b739`；M2 的执行合同、设置、专注陪伴与独立验收环境已实现并通过聚焦验证。
  当前独立执行器支持文字和可选应用事件，截图输入明确不支持；未来 Agent 未实现。真实 Core/插件进程、桌面协议与 Edge 联接
  检查已通过，原生窗口视觉及音频尚未验收。下一主线为 M3，详细范围和证据见总计划第 12 节。
