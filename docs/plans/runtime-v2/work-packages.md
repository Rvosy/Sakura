...
执行合同、进程绑定和取消作为开发基础保留。M3/M4 的实现尚未迁出，后续围绕已有消费者逐项推进。
M5 已完成普通服务绑定、TTS/ASR 旧任务隔离及可选便签记忆和显式查询修正。
本轮用 Mem0 与便签记忆统一现有插件设置的 Collection 生命周期：保护所有集合草稿、隔离角色切换，保留同角色重启草稿。
复杂功能优先使用已有插件设置容器，按实际缺口扩展；本轮不增加 SDK 字段、自定义渲染、业务插件或模式选择。
该计划不改变下表尚未核对的历史验收状态。
...
- 开放插件生态 M1 已提交 `2097b739`。M2 的历史实现 `f9fde091` 曾提供执行器选择，现按用户要求撤回主设置和正常聊天入口，
  ...
  当前日常验收复用已有数据、默认 Assistant 和插件自身的对话规则文本。M3/M4 记录尚未实施的迁移范围；历史范围见总计划第 12 节。
- 开放插件生态 M5 前两项已交付：公开 `context.bind()`，让 TTS Hub 的旧 job 查询/取消固定到原 Provider 进程；
  增加可选[便签记忆](../../specs/runtime-v2/fact-memory.md)，由插件自己的 Collection、私有 SQLite、Context 和只读工具完成管理与召回。
  两项的测试和 Git 节点已分别记录。日常使用由用户通过已有数据启动器安装、启用可选插件，
  不自动改写原角色、模型、插件开关或 Mem0 数据。范围与验收见总计划第 13 节。
- 用户便签试用经本地 Trace 核实已在正常 Assistant 生效；`0.1.1` 修正显式工具搜索漏查已设关键词便签正文的问题。
  修正提交 `4bd53bc5`，相关 15 项自动回归通过，包含卸载重装保留条目与重新启用；新版 ZIP 已生成。
  现有 ASR Hub/Provider 已复用服务绑定，固定录音任务实例并隔离迟到结果。6 个真实进程回归先红后绿；
  `journey-asr` 通过 3/3，覆盖 Python 45 项、Rust 11 项、前端 22 项；1 项真实麦克风测试未运行。
  这完成 ASR 绑定与便签搜索修正，M5 的后续能力仍由实际消费者推动。
- 现有插件设置的 Collection 生命周期已统一：所有集合草稿阻止角色切换，待切换与切换期间暂停查询/写入，
  异步删除确认后复核状态；实际角色变化清理页面状态，同角色 Core 重启保留仍存在集合的草稿。
  自动回归覆盖普通插件与 Memory 页面，并通过屏障验证重叠刷新及本地切换不会互相提前解锁；原生窗口人工验收待进行。
  后续只改造已有能力，复杂功能优先使用现有插件设置容器。
