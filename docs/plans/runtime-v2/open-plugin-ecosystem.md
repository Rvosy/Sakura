...
M1 已修正 Context 用途分类及宿主裁剪；下一条主线是打通可替换的执行入口，再迁出默认模型和对话策略。
...
| M1：Context 去语义分类 | 删除用途分类；Host 传原始贡献，默认消费者承接兼容裁剪；更新 describe、单输入示例及规范 | 任意插件自行组织上下文，原角色信息可共存；能力查询如实反映接口变化 | 已实施并通过相关回归 |
...
review 修订提交 `abc5420d` 只更新文档。后续用户已授权开始实现并逐里程碑提交；M1 的代码、示例和规范现已同步，
M2 正在实施。真实验收场景将提供不依赖 API 配置的离线互动插件，使用原输入框完成任务、查看进度和停止。
