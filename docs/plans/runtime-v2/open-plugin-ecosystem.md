...
底层执行合同与“专注陪伴”开发样例保留。当前日常入口使用已有数据和默认 Assistant，插件通过自己的上下文设置增强对话。
...
普通插件已经可以提供自有 Service。出现两个独立插件实际组合业务的需求时，再补足有效绑定或注销等必要缺口，
...
| M5：通过真实插件补能力 | 记忆、协作、互动、TTS/表现操作 | 按实际消费者确定范围 |
| M6：插件界面 | 窗口、服务桥、命令及页面等可能形式 | 待具体用户需求与界面方案，不预定入口 |

当前工作是收回预设的产品呈现，恢复默认 Assistant 的正常使用，并用已有角色包、模型和插件自身上下文设置验收。
...
以及进程绑定和取消的开发回归继续成立。真实日常数据里的角色、模型、插件开关和上下文文本不为验收自动改写。
