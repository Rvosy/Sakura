...
2. Host 保留来源、Provider 身份、登记有效性、取消和有界 JSON 传输检查，交付可承载的原始贡献。
   当前最多 16 项、每项 8192 字符及旧可选内容截断行为，移到默认消费者的兼容处理处；
...
M1 先在现有进程内完成上述责任迁移，M4 再把默认消费者代码迁到插件；不能把仍在 Host 的截断政策留到 M4 后。
...
但 M2 开始实现前按下表确定交接规则；这些是待实现约定，不是当前 API。
...
M2 使用独立无副作用执行器持续报告进度，以事件/屏障证明取消到达实际后台工作并使其退出；再重复读取终态、
重载同 ID 插件并释放旧结果，验证助手历史与播放只提交一次或完全未提交。不能只断言界面显示 `cancelled`。
...
| M1：Context 去语义分类 | 删除用途分类；Host 传原始贡献，默认消费者承接兼容裁剪；更新 describe、单输入示例及规范 | 任意插件自行组织上下文，原角色信息可共存；能力查询如实反映接口变化 | 待实施 |
| M2：执行入口可替换 | 公共执行合同、选择与就绪解耦；固定进程及后台操作绑定；实际取消与终态去重 | 没有 API 配置也能互动、查看进度和停止；重载及重复读结果不重复写历史或播放 | 下一主里程碑 |
| M3：模型适配器可替换 | 不同协议适配器；中立 model_slots 路径及旧接口兼容；聊天和辅助消费者共用；公共 Trace 桥 | 聊天与记忆整理可用同一非 HTTP 提供者，旧配置可用，实际请求可追踪 | 待实施 |
...
分支中已有 `d2118925` 的 Context 产品实现；本次 review 修订只更新总计划、方向提案和开发接口提示，未修改产品代码。
下次开发从 M1 和 M2 开始，以真实替换为主要证据。
