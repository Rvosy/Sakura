...
M5 已通过 TTS Hub 的旧任务隔离和可选“便签记忆”验证公共能力，随后完成现有 ASR 长任务绑定和便签显式搜索修正。
本轮继续用 Mem0 与便签记忆检查已有设置容器，将 Collection 的草稿、角色切换和进程重绑定保护统一为通用行为。
M3/M4 的默认模型与对话实现迁移尚未实施，后续围绕已有消费者逐项推进。

主设置保留现有导航、布局和选择控件，底层设置机制按现有消费者的需要调整。插件配置和复杂功能优先放在已有插件设置容器中；
将来结构化贡献不足以呈现某项实际功能时，先考虑扩展该容器的渲染能力。当前没有自定义渲染消费者，不新增此能力。
独立页面、窗口或 Agent 模式都不是插件生态改造的前置条件。
...
| 范围 | 当前已经具备 | 后续需要解决 |
|---|---|---|
| 插件运行 | API v4、独立进程与依赖根、具名 Service、配置、资源和生命周期；`context.bind()` 固定一次操作的服务进程 | 用现有消费者检查公共调用边界，按实际缺口调整 |
| Memory | Mem0 拥有存储、检索、整理、Context、工具与管理页贡献；便签记忆提供独立 SQLite 和正文/关键词显式搜索 | 用两个现有消费者验证 Collection 的通用设置行为，不另建业务样例 |
| TTS | Core 消费公共 `sakura.tts`；Hub 可替换，Provider 以公开合同接入不同引擎 | 已用显式绑定修复 crash→reload 后旧 job 落到新进程的问题 |
| ASR | Core 消费公共 `sakura.asr`，录音授权与读取租约由 Host 管理 | 已用现有服务绑定固定 Hub 和 Provider，隔离重载间隙的旧调用与旧结果 |
...
| 默认聊天与执行基础 | 正常入口使用默认 Assistant；执行服务与进程绑定保留供开发消费者验证 | 模型与默认对话实现尚未迁出；后续根据调用链缺口推进，不要求先设计模式入口 |
| 工具与权限 | 有工具发现、可见性过滤、调用来源及进程隔离 | 默认 Assistant 的工具选择需要与新增 Agent 能力分开；授权设计由未来实际消费者推动，进程隔离不等于 OS 权限隔离 |
| 插件界面 | 结构化设置、Collection、表现渲染器与工坊编辑器已有接入；Collection 已共用角色切换保护 | 按现有消费者暴露的缺口调整容器，不预建独立窗口 |
...
正常应用的初始化仍创建默认 Assistant；模型、Context 与工具的进一步迁移按真实调用链中的依赖逐项确定。
...
模型和默认能力拆分不要求先引入 Agent、独立窗口或选择器；里程碑编号不规定产品入口，也不是结构调整的审批顺序。
...
M3/M4 的以下两条链路尚未实施。本轮先处理已有设置容器暴露的结构问题；后续迁移根据现有消费者、调用链和维护收益确定范围，
不要求先交付新的业务插件或让用户提出界面方案。以下条件用于判断迁移是否成立，不预定新的模型页、模式选择或默认能力组合。
...
M4 还要证明默认对话能被小插件增强：保持默认执行器、模型和现有工具，通过默认执行器拥有的少量公共入口组合上下文与输出处理。
新增输出处理链时，转换后的结果应一致进入历史、气泡和 TTS；移除任一增强插件，其他能力继续工作。
验证优先复用现有插件，需要构造边界分支时使用隔离夹具，不要求随迁移交付两个新的业务插件。
第三方无需复制默认对话插件，Core 也不建立中央 Hook 平台。
...
- **Memory：**已提供可选 `fact_memory`，由用户通过插件自己的 Collection 维护便签，用独立 SQLite 按角色保存。
  Context 按关键词或轻量文本匹配，只读 `fact_memory_search` 提供显式查询；两者最多返回 5 条、正文合计不超过 2000 字符。
  `0.1.1` 将显式工具查询与自动 Context 匹配分开：工具按正文或关键词做字面子串查询，填写关键词不屏蔽正文。
...
- **TTS：**沿用现有 Hub/Provider 和播放边界，已通过普通 `context.bind()` 固定 Provider 的实际进程，
  `begin/poll/cancel` 不跨重载接到新 job；新任务再显式绑定。声线查询、选择和资源适配由 TTS 能力完成，
  工作室的厂商资源适配随资源消费者逐项开放。
- **ASR：**现有语音输入复用相同绑定，固定一次录音的 Hub 与 Provider，成功结果进入草稿前仍检查实例有效性。
  音频授权和读取租约保持原合同，不新增引擎、模式或主设置。
...
普通插件已经可以提供自有 Service。TTS Hub/Provider 的旧 job 隔离需求已补足显式进程绑定，
不要求主程序预先认识服务方法和内容。耗时处理通过显式调用得到结果，通知事件只说明已发生的事实。

本轮先统一现有 Settings Contribution 的 Collection 生命周期，不按 `surface=memory` 决定是否保护草稿与隔离角色数据：

- 任一插件 Collection 的新建或编辑草稿都会阻止选择另一角色。Collection 的保存仍走自身回调，不并入普通字段的“应用”。
- 角色选择尚未应用或正在切换时，所有 Collection 暂停查询和写入；异步删除确认返回后再次核对当前 Collection 与切换状态。
- 实际角色变化时统一关闭编辑器、失效在途查询并清空 Collection 页面状态。同角色 Core 重启保留仍存在 Collection 的草稿，旧请求不得提交结果或重新执行写入。
- 普通 Collection 删除使用“删除记录”标题，确认正文沿用插件声明；Memory 页保留其领域文案。

这一步复用 Mem0、便签记忆与隔离前端夹具，不增加 SDK 字段、控件或业务插件。Collection 的存储范围和数据格式仍由插件决定，
宿主只保证未提交编辑不会被角色切换误带到新实例。

以下场景用于说明将来可能遇到的公共能力缺口，不是本轮交付清单，也不要求先实现对应业务插件：
...
| 日历、棋盘或记忆图谱 | 现有插件设置中的合适呈现；必要时补充渲染与受控服务桥 | 宿主管理容器与生命周期，插件实现业务内容和交互 |
...
复杂功能优先使用现有插件设置容器；现有字段、Action 与 Collection 能满足需求时，直接复用它们。
实际消费者需要自定义呈现时，先评估在同一容器内扩展渲染，沿用 renderer/editor 的资源加载、身份与清理经验；本轮不实现自定义 HTML 或 WebView。
只有现有容器确实无法满足独立使用需求时，再讨论新页面或窗口。未来视图关闭不应自行终止插件后台工作，停用插件仍必须清理其任务。
服务桥绑定实际插件与服务，只提供已开放的操作；继续采用可信本地插件模型，不宣称为恶意代码沙箱。
...
| M3：模型适配器可替换 | 不同协议、中立模型槽、辅助消费者及公共 Trace 的设想 | 未实施；保留现有模型配置与使用流程，迁移现有消费者 |
| M4：默认对话能力拆分 | Agent/Context 策略、工具消费、小插件增强及 Trace 关联的设想 | 长期方向，未实施；不要求先新增业务插件或选择入口 |
| M5：通过真实插件补能力 | 普通服务绑定与 TTS/ASR 任务隔离；可选便签记忆的 Collection、Context 和工具 | TTS、便签初版、ASR 绑定和便签查询修正已完成相关回归，证据见第 13 节 |
| M6：插件界面 | 通用设置容器、按需呈现与生命周期 | 已统一现有 Collection 的角色与进程生命周期；后续按实际需求扩展容器 |

本轮处理现有插件设置容器的通用 Collection 行为，默认 Assistant 继续使用正常入口。
日常体验通过已安装插件自身设置检查；自动验证使用现有合同和隔离数据，不要求用户安装第二个新样例。
里程碑按实际依赖推进。后续只改造已有能力，用现有插件和隔离夹具验证，不为完成计划另建业务插件。
...
本轮自动回归已覆盖普通插件与 Memory Collection 的草稿阻断、待切换时查询/写入暂停、删除确认后的状态复核，
以及实际角色变化清理与同角色重启保留。重叠通知通过屏障安排两种完成顺序，确认本地切换和外部刷新不会互相提前解锁。
原生窗口人工验收仍待进行，步骤见[插件设置容器验收](../../devdocs/PLUGIN_ECOSYSTEM_ACCEPTANCE.md#插件设置容器的改造验收)。
既有验收检查正常聊天仍使用默认 Assistant、历史选择字段不接管入口、插件上下文从自身设置生效，
以及进程绑定和取消的开发回归继续成立。M5 分别验证普通服务的旧绑定拒绝重载进程、TTS 的同名 job 隔离，
以及便签的新增、查询、修改、重启保留、删除、停用和角色分区。真实日常数据里的角色、模型、插件开关和上下文文本不为验收自动改写。

下面是后续方案的判断标准，不是本轮已承诺的界面或拆分工作。替换能力应有实际调用链，验证可复用现有消费者和隔离夹具，
...
- 插件设置容器：独立于聊天的功能不依赖模型配置；草稿、关闭、重开、停用和资源回收符合公共契约。
  未来若扩展自定义呈现或独立页面，再补对应真实桌面验证。
...
撤回选择入口时，M3/M4 尚未实施。后续结构调整继续依据实际消费者推进，复杂呈现优先扩展已有插件设置容器。
...
初版交付包为 `dist/fact_memory-0.1.0.sakplugin.zip`。不把包的准备或本文范围视为测试和人工验收通过。
...
初版验证：服务绑定与 TTS 相关 91 项通过，修复前真实进程测试已复现旧取消被重载后的新任务接受；
...
后续用户使用已有角色、模型和便签试用后，经本地 Prompt Trace 与工具调用记录核对，确认便签已进入正常
Assistant 的上下文并参与回答。试用同时发现显式搜索沿用自动 Context 规则，导致已填写关键词的便签无法
按正文查询。`0.1.1` 将工具改为正文或关键词的字面子串搜索；自动 Context 的原匹配策略保持不变。
新包为 `dist/fact_memory-0.1.1.sakplugin.zip`。通过现有卸载入口保留数据，再安装新版并重新启用；
原数据库和条目 ID 保持可读，启用状态不保留。验收记录不包含用户的便签或聊天原文。
该修正已存档为 `4bd53bc5`，便签聚焦与真实 Core IPC 共 15 项通过，新版 ZIP 已生成。
实际 Core 测试覆盖卸载、重装后保持停用、重新启用、原条目 ID 与正文保留，以及重启后按正文查询。

随后完成现有 ASR 长任务绑定：Hub 使用公开 `context.bind()` 固定 Provider，Core 固定录音开始时的 Hub，
收到成功结果后及首次向 UI 交付前仍核对两端实例。任务不因重载被交给同名新进程，音频授权和租约按原链路回收。
这是已有语音输入消费者的修正，不新增样例插件、通用事件总线、主设置或模式入口。回归范围见
[ASR 规范](../../specs/runtime-v2/asr-voice-input.md)和[生态验收指南](../../devdocs/PLUGIN_ECOSYSTEM_ACCEPTANCE.md)。

新增 6 个真实进程回归均先在修复前失败，修复后通过：2 个暴露 scope 检查后仍由动态代理把旧 `poll/cancel`
发给新进程，2 个暴露识别返回后缺少来源复核，另 2 个暴露成功暂存后、首次 UI 交付前重载仍会交付旧文本。
测试确认重载后的 Provider 未收到旧 `jobId`，失败结果不含转写正文，音频资源按租约回收。
最终 `journey-asr` 通过 3/3：Python 45 项（含上述 6 项）、Rust 11 项、前端 22 项通过。
Rust 的 1 项真实麦克风测试保持 ignored；该轮未使用真实麦克风或运行真实识别模型，也未重新进行原生窗口人工验收。
这些结果完成 ASR 绑定与便签搜索修正，不代表 M5 的所有后续场景或整个插件生态已完成。
