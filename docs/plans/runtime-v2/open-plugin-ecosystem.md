...
M5 已通过 TTS Hub 的旧任务隔离和可选“便签记忆”验证公共能力。本轮已把相同绑定用于现有 ASR 长任务，
并完成便签真实试用中发现的显式搜索漏查正文修正。
...
| Memory | Mem0 自己拥有存储、检索、整理、Context、工具与管理页贡献；便签记忆以独立 SQLite 提供第二实现 | 本轮修正显式工具查询，让已设关键词的便签也能按正文搜索 |
...
| M5：通过真实插件补能力 | 普通服务绑定与 TTS/ASR 任务隔离；可选便签记忆的 Collection、Context 和工具 | TTS、便签初版及本轮 ASR 绑定、便签查询修正已完成相关回归；后续按实际消费者扩展，证据见第 13 节 |
| M6：插件界面 | 窗口、服务桥、命令及页面等可能形式 | 待具体用户需求与界面方案，不预定入口 |

本轮已完成 M5 中的 ASR 绑定和便签查询修正，保留默认 Assistant 的正常入口。日常验收使用已有角色包、模型配置，
...
本轮已完成现有 ASR 长任务绑定：Hub 使用公开 `context.bind()` 固定 Provider，Core 固定录音开始时的 Hub，
...
Rust 的 1 项真实麦克风测试保持 ignored；本轮未使用真实麦克风或运行真实识别模型，也未重新进行原生窗口人工验收。
这些结果完成本轮两个修正，不代表 M5 的所有后续场景或整个插件生态已完成。
