...
M1 已修正 Context 用途分类及宿主裁剪，M2 已接通可替换执行入口并交付离线“专注陪伴”插件。
下一条主线是 M3 模型适配器替换，随后由 M4 迁出默认对话策略；当前默认对话仍由本地适配器包装。
...
| 默认聊天与执行入口 | M2 已让正常入口消费选中的执行服务；默认模型客户端、Agent 和管线由本地适配器包装 | M3 迁出模型能力，M4 迁出默认对话策略并清理私有直连 |
...
M2 已实现：没有模型配置、默认对话实现未被选用时，独立插件可以通过原输入框完成正常互动。
“专注陪伴”提供真实计时、进度和停止，设置、初始化、受理、取消、历史与输出已接入现有产品链路。
当前合同见 [Plugin Runtime v4 §6.5](../../specs/runtime-v2/sakura-plugin-runtime-v4.md#65-可替换互动执行器)。
独立执行器支持 `text`，可额外声明 `event`；截图和其他图片输入尚未开放，收到这类请求会明确拒绝。

当前调用链与剩余迁移：

| 位置 | M2 已实现 | 后续范围 |
|---|---|---|
| [assistant_adapter.py](../../../app/core_host/assistant_adapter.py) | Session 可绑定独立执行服务；默认管线由本地适配器承接 | M3/M4 迁出默认模型及对话实现，删除相应私有对象依赖 |
| [core_config_reader.py](../../../app/config/core_config_reader.py)、[server.py](../../../app/core_host/server.py) | 所选执行器独立检查就绪，模型缺失不关闭无模型执行器；切换在当前操作退出后生效 | M3 将默认模型就绪与配置消费迁到公共模型能力 |
| [real_chat.py](../../../app/core_host/real_chat.py) | 独立执行器使用公开请求、进度和结果，共用历史及输出提交边界 | M4 迁出默认对话内部的 Agent 动作解释与 Trace 关联 |
| [plugin_application.py](../../../app/core_host/plugin_application.py)、[plugin_runtime_application.py](../../../app/core_host/plugin_runtime_application.py) | 无 AgentRuntime 的 Session 也可绑定角色表现与插件服务 | M4 让默认对话经公开入口消费 Context 与工具 |
| [plugin_host_services.py](../../../app/core_host/plugin_host_services.py) | 既有工具与 Context 登记桥继续可用 | M4 随真实消费者迁移，补当前操作内的工具查询/执行和原始贡献收集；M2 未开放这条工具消费链 |

第一版执行合同通过 `describe/begin/read/cancel` 覆盖就绪、受理、读取进度/结果和取消。
插件使用已有跨进程 Service 机制登记候选，长任务快速受理并后台执行，宿主读取有界快照；
不跨进程传 Python 对象、生成器或 Agent 私有动作。以下交接规则已纳入当前合同，字段和错误行为以 §6.5 为准。

| 边界 | 最小约定 |
|---|---|
| 加载与就绪 | 插件已加载、Service 已注册、执行器业务就绪分别表达。就绪检查通过且进程绑定仍有效，才发布可用 Session。模型或其他业务配置无效时，设置仍可访问；独立执行器只等待自己的依赖，不等待无关模型或 MCP。 |
| 受理与绑定 | 宿主在派发前关联 operation、会话/角色、Core generation、选中服务及具体插件进程实例；任务 ID 仅在该实例内解释。同一 operation 重复受理不启动第二份工作；同 ID 插件重载后不接管旧任务。 |
| 后台身份 | `context.caller_id` 只属于当前 RPC，不自动进入新线程。执行器受理时保存经当前调用验证的关联，显式传给自己的后台工作；宿主收到后续结果仍校验实际来源和活动绑定，不能仅凭业务参数中的调用者或 operation ID 接受提交。 |
| 超时与取消 | Service 调用超时只表示这次调用未按时返回，不能据此宣称任务停止或重新受理。通过原 operation 的关联查询或取消，即使未收到插件任务 ID 也要能定位本次工作；取消由执行器传递到下游模型、工具或子进程，并报告真实停止或未确认的状态。 |
| 协议与调用故障 | 每次执行服务调用期限为 1 秒。查询超时保持忙碌；协议损坏或非超时 RPC 故障时，先回收原进程及其依赖消费者，再结束操作。正常业务失败只结束当前任务，不自动重启或回退。 |
| 清理失败 | 等待者收到明确清理错误，原实例保留并禁止重启；聊天报告停止未确认，保留操作占用，不能伪称取消或接受新工作。用户退出后由既有 Core 生命周期回收进程树。 |
| 进度与终态 | 进度仅更新活动任务展示，不写助手历史或触发播放。执行器的完成结果是待提交候选，最终成功、失败或取消由宿主现有操作边界判定；插件内部子任务不升级为宿主任务图。 |
| 提交与播放 | 最终结果先校验身份、绑定和结果格式，再按既有提交边界写助手历史、取得唯一完成资格并交付可播放输出。重复读取同一终态不得重复写历史或重复触发播放；NOOP 保留不写助手条目的语义。输入是否已落盘继续按当前链路处理，失败不伪造助手回复。 |
| 取消竞态 | 提交前取消撤销候选资格，迟到结果不能重新成功；历史提交及完成资格已经取得后，不回写成未发生。播放阶段的停止由原播放控制处理，取消不撤销已经发生的播放或其他外部效果。 |
| 选择切换 | 用户申请切换与实际生效分别表达。在途操作保留原实例绑定；切换在其退出后应用，或由用户先取消。失败显示所选实现和原因，不静默改回默认实现。 |

沿用现有单轮忙碌、操作终态和提交保护。终态去重只需覆盖当前操作的结果消费，不新增持久任务库、自动重放、
通用任务图或外部效果“恰好一次”保证。取消无法确认完成时，不把仍在运行的工作当作空闲或悄悄在新实例重启。

M2 已用真实独立进程和无副作用执行器验证进度、取消及提交。事件/屏障证明取消到达实际后台工作并使其退出；
重复读取终态、重载同 ID 插件及释放旧结果的回归覆盖助手历史和播放交付不重复提交。原生窗口与音频体验仍需人工验收。
...
M2 已以无副作用的独立执行器验证选择、进度和取消；M4 将以无副作用工具证明两个执行器可以选择不同工具，
...
M2 已接通执行入口，下一主线为 M3 模型能力迁移；完成后再推进 M4 默认对话迁移。以下两条链路尚未实施。
...
| M1：Context 去语义分类 | 删除用途分类；Host 传原始贡献，默认消费者承接兼容裁剪；更新 describe、单输入示例及规范 | 任意插件自行组织上下文，原角色信息可共存；能力查询如实反映接口变化 | 已完成，提交 `2097b739` |
| M2：执行入口可替换 | 公共执行合同、选择与就绪解耦；固定进程及后台操作绑定；实际取消与终态去重；专注陪伴及独立验收环境 | 没有 API 配置也能互动、查看进度和停止；重载及重复读结果不重复写历史或交付播放 | 已实现并通过聚焦验证，原生窗口/音频待验收 |
| M3：模型适配器可替换 | 不同协议适配器；中立 model_slots 路径及旧接口兼容；聊天和辅助消费者共用；公共 Trace 桥 | 聊天与记忆整理可用同一非 HTTP 提供者，旧配置可用，实际请求可追踪 | 下一主里程碑，待实施 |
| M4：默认对话真正插件化 | Agent、Context 策略迁出；公开工具消费与独立选择；小插件增强入口；执行器 Trace 关联；清理私有直连 | 默认对话同时使用上下文增强与输出转换；替代执行器使用所需工具；默认 Assistant 不自动获得新增高级工具 | 待实施 |
...
M1 已单独提交，M2 也作为完整节点归档。后续继续按里程碑提交实现、消费者、兼容、必要测试和对应文档，
保持每个节点可审阅。提交和推送按用户授权范围执行，发布单独处理。

未来命令执行 Agent 单独开发，不加入已交付的 M1/M2 或下一主线 M3 的实现范围。当前仅按第 5.1 节检查接口，
...
- 无模型执行器：缺少 API 配置仍可运行和配置，模型配置热更新不注销它；事件或屏障证明取消使实际后台工作退出。
...
M2 的真实 Core/插件进程、桌面协议与浏览器联接验证范围见第 12 节；其通过不代表 M3/M4 已迁出默认实现。
...
本次结合对 `main@d24a3e73 → codex/open-plugin-ecosystem@878eb76d` 的 review 补齐六项交接：
Context 传输与消费限制、执行器生命周期、模型槽解耦、默认对话的小插件增强、公共 Trace、ADR-0033 的部分替代。
Trace 桥提前随 M3 模型迁移提供，M4 完成执行器关联；主线顺序保持 M1 → M2 → M3 → M4。

它取代上一方案“先扩充 Context/处理链/业务组合，再长期等待模型和执行器替换”的顺序。
原先已完成的 Context 工程作为可复用实现及回归基础；分类设计需要修正，不视为总目标已经达成。

review 修订提交 `abc5420d` 只更新文档。用户随后授权实现并逐里程碑提交；M1 已在 `2097b739` 完成代码、示例与规范同步。
M2 的公共执行入口、设置、离线“专注陪伴”插件和独立验收启动环境已实现并通过聚焦验证。
下一主线是 M3；M4 尚未实施，默认对话仍是对现有实现的本地包装，未来 Agent 也尚未实现。

M2 已完成的验证：

- Python 聚焦回归已覆盖 `plugin_executor_chat`、`executor_core_protocol`、`bound_service_lifecycle`、
  `executor_settings` 和 `focus_companion`。其中包含真实 Core 独立进程 IPC、真实插件进程、取消、重载与提交保护。
- Rust `core_host` 73 项通过，1 项沿用既有 ignored；`chat_bridge` 5 项通过，`cargo build` 成功。
- JavaScript 设置测试 18 项、进度测试 32 项通过。Edge 加载实际设置页 HTML 和控制器，并以模拟原生 IPC 检查设置联接，结果通过。
  进度链另经逐层静态复核与自动测试；这项浏览器检查不覆盖进度显示，也不代表已经运行原生 Tauri 窗口。
- 独立验收目录已通过实际安装器安装示例，使用无 API 配置的 Session 与真实插件进程验证就绪；
  立绘资源可加载，Timeline 初始化为空。准备和操作步骤见[初版验收说明](../../devdocs/PLUGIN_ECOSYSTEM_ACCEPTANCE.md)。

该环境可从原输入框体验十秒计时完成、一分钟途中停止、在途切换、无 API 时切回专注，以及停用/再启用后的同服务重试。
进度只作临时显示，最终结果只提交一次。“对话规则”的 Context 体验需要自行配置模型；离线计时不消费 Context。
原生窗口视觉、音频和这些交互的人工验收尚未执行，不能由自动回归或 Edge 联接检查代替。
