...
本文依据 2026-09-13 的讨论和 Sakura `d24a3e73` 源码制定，现已开始实施。初版完成阶段 A 中的统一 Context：规则与资料区分、本轮复用、必需内容和回调失败策略，以及可安装的“对话规则”示例。当前合同见 [Runtime v4 §6.4](../../specs/runtime-v2/sakura-plugin-runtime-v4.md#64-context-行为贡献)。A 的其他部分及 B–E 仍为计划，下文未标明落地的新接口和页面行为均为设计建议。
...
继续使用 `sakura.host.context.register()`。初版已实现片段用途和完整性字段；本轮范围通过注册描述的 `scope: "turn"` 指定，作用于整个回调结果：
...
- 注册描述通过 `failurePolicy: "abort"` 声明失败时中止，默认 `skip` 跳过。片段通过 `required: true` 声明成功结果必须完整装入预算；回调直接失败时无法依赖这个尚未返回的字段。
...
不在方案阶段给出精确工期。C 的跨进程模型切片和 E 的原生容器验证完成后，才能根据实际缺口估算工作量。初版只完成 A 的统一 Context 切片，输入输出转换、历史扩展与记忆筛选尚未实施，A 不能整体标为完成。
...
- 本文保留后续计划，不把未实现行为写入当前规范；实施顺序用于说明依赖，不是开发权限或审批门槛。
...
已阅读关联任务“评估插件形态接口适配”及用户粘贴的讨论，对照上述源码和当前规范；外部参考为 DeepSeek Harness 官方 README 与架构文档。方案基线为 `d24a3e73`。

初版修改 Host、SDK、上下文组装、预算、提示词渲染、错误归因和真实聊天链。自动验证覆盖独立插件进程到实际 HTTP 请求、必需贡献失败不调用模型及不写助手历史、安装启停与配置持久化，以及旧 Context 行为兼容。示例见[对话规则](../../../plugins/optional/context_rules/README.md)。原生设置窗口操作和真实模型遵从效果仍需实际体验，不能用自动夹具结果替代。
