...
5. 每次 Shell 启动覆盖 `data/logs/memory-initialization.jsonl`，串联 owner/preload、embedding 子进程
   阶段、Core Memory 请求、deadline、设置关闭与重开；只写固定事件、耗时、状态和错误类别。
...
- 诊断时间线必须有界、每次启动清空，只记录当前运行；secret/path/content scan 为零，日志失败不影响
  Memory、设置或普通聊天。
