...
官网、控制台与遥测源码统一在 `services/sakura/` 管理。官网的 Astro / Starlight 工程在
[`site/`](../../services/sakura/site/README.md)，服务进程和 Nginx 配置对应关系见
[`deploy/`](../../services/sakura/deploy/README.md)。官网独立构建，发布时保留共享目录下的 `service/` 和隐藏配置；
不能将官网构建目录整体替换为在线站点根目录。官网发布不隐式发布版本清单或重启 Python 服务。
...
[ADR-0055](../adr/0055-domestic-updater-manifest.md)。
