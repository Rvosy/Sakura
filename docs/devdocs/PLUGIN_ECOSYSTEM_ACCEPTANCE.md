...
本轮 M5 用两个实际消费者推进公共能力：普通插件可显式绑定服务进程，TTS Hub 据此隔离重载前后的任务；
便签记忆用自己的存储、Collection、Context 和只读工具提供另一种记忆方式。两项分别保存工程证据，
...
使用上面的已有数据启动器，由用户安装 `dist/fact_memory-0.1.0.sakplugin.zip` 并启用“便签记忆”。
...
没有关键词时使用轻量文字匹配，完整规则见[便签记忆规范](../specs/runtime-v2/fact-memory.md)。
...
自动结果与已有数据中的原生窗口和真实模型体验分别记录；本文不预填通过状态。
