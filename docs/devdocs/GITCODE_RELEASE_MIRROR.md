...
- [镜像 35001855469](https://github.com/Rvosy/Sakura/actions/runs/35001855469)，提交 `7f19d27f`、attempt 1、Ubuntu：代码同步完成后，读取 `/releases/latest` 返回 `GITCODE_API_HTTP_400`，原文为 `No latest release found`。后续修复仅将这个接口的明确空结果识别为尚无正式版本，其他 HTTP 400 继续失败；回归测试覆盖这两种响应，整理后的发布链路测试通过。
...
```text
gh workflow run mirror-gitcode-release.yml --ref <待测分支> -f tag=v1.1.2 -f mode=rehearsal
```
